    ...
    <div className="container-page py-16">
      <div className="text-center max-w-xl mx-auto">
        ...
        <p className="text-muted mt-3">
          Choose a plan that fits your hiring needs and start reaching qualified
          candidates today.
        </p>
      </div>
        ...
        {plans.map((plan) => (
          <div
            ...
            className={`rounded-2xl border p-7 flex flex-col ${
              plan.highlight
                ? "border-brandGreen bg-brandGreen text-white shadow-cardHover"
                : "border-border bg-white"
            }`}
          >
            <h3
              className={`text-sm font-semibold ${plan.highlight ? "text-white/80" : "text-muted"}`}
            >
              {plan.name}
            </h3>
            <p className="text-3xl font-extrabold mt-2">{plan.price}</p>
            <p
              className={`text-sm mt-2 ${plan.highlight ? "text-white/70" : "text-muted"}`}
            >
              {plan.desc}
            </p>
            ...
            <ul className="mt-6 space-y-2.5 flex-1">
              {plan.features.map((f) => (
                <li key={f} className="flex items-center gap-2 text-sm">
                  <Check
                    className={`h-4 w-4 shrink-0 ${plan.highlight ? "text-white" : "text-brandGreen"}`}
                  />
                  {f}
                </li>
              ))}
